...
def get_bot_response():
    with sqlite3.connect("db1.sqlite3") as db:
        ...
        if(userText=='Veg pizza' or userText == 'Capricorn pizza' or userText=='Hot Pizza' or userText=='Bella Italia'):
            if(p_name == '' ):
                if (userText == 'Veg pizza'):
                    total += 120
                elif (userText== 'Capricorn pizza'):
                    total += 150
                elif (userText== 'Hot Pizza'):
                    total += 200
                elif (userText== 'Bella Italia'):
                    total += 250
                p_name=userText
            else:
                if (userText == 'Veg pizza'):
        ...
        if(flag == 1):
            ...
            p_name=''
            total=0
            return 'Confirm your order...........Your Order Id:'+ str(rows[0][0])   + ' Your Order List :'+ str(rows[0][4]) + 'Total amount :'+ str(rows[0][5])
            #return 'Your pizza order has been placed successfully. Your order id: '+str(rows[0][0])+' \t Your LIST:  '+str(rows[0][4]) +'  Total amount  '+str(rows[0][5])
        elif(flag==2):
            myid=int(userText)
            ...
            if cursor.fetchone()[0] == 0:
                flag=0
                db.commit()
                return 'Your order id is not found'
            ...
            if(res=='Give your name , phone , address (given format)'):
                flag=1
            elif(res=='Enter your order id'):
                flag=2
            return res
